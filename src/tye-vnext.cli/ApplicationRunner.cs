...
public class ApplicationRunner : IApplicationProcessor
{
    private readonly ILogger _logger;

    public ApplicationRunner(ILogger logger)
    {
        _logger = logger;
    }
    ...
    private async Task BuildAndRunProjects(Application application)
    {
        ...
        // TODO: Each individual service should be handled by it's own IApplicationProcessor
        ...
        if (projectGroups.Count > 0)
        {
            ...
            if (buildResult.ExitCode != 0)
            {
                ...
                throw new Exception($"Building projects failed with exit code {buildResult.ExitCode}: \r\n{buildResult.StandardOutput}");
            ...
            while (!processInfo.StoppedTokenSource.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    var processInfo = new ProcessSpec
                    {
                        ...
                        // OutputData = data => { service.Logs.OnNext($"[{replica}]: {data}"); },
